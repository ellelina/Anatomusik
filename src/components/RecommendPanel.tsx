  ...
  return (
    <div className="ml-4 mr-1 mt-1 mb-3 rounded-xl p-5" style={{ border: "1px solid rgba(180,200,255,0.12)", background: "rgba(150,180,255,0.04)" }}>
      ...
      {!loading && recommendations.length === 0 && !error && (
        <p className="text-sm text-neutral-500 py-2">No matching tracks found.</p>
      )}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
          {(rec.genres || []).map((g) => (
            <span key={g} className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-purple-500/10 text-purple-300">
              {g}
            </span>
          ))}
        </div>
        <p className="text-[11px] text-neutral-500 mt-1.5 leading-relaxed">{rec.whyMatch}</p>
      </div>
    </a>
  );
}
